...
export default function EditCourse({
  ...
  const deleteModule = (moduleId: string) => {
    updateModules(
      editingCourse.courseData.modules.filter(
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-4">
            {editingCourse.courseData.modules.map((m) => {
              ...
              return (
                <div key={m.moduleId} className="group border border-slate-200 rounded-xl overflow-hidden bg-white hover:border-indigo-200 transition-all shadow-sm">
                  {/* MODULE HEADER */}
                        ...
                        <div key={s.submoduleId} className="bg-slate-50/50 border border-slate-100 p-4 rounded-lg group/sub">
                          <div className="flex justify-between items-center mb-3">
                            ...
                            <div className="flex gap-1 items-center opacity-0 group-hover/sub:opacity-100 transition-opacity">
                              <button onClick={() => setEditingSubId(s.submoduleId)} className="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors">
                                <Pencil size={14} />
                              </button>
                              <button onClick={() => deleteSubmodule(m.moduleId, s.submoduleId)} className="p-1.5 text-slate-400 hover:text-red-600 transition-colors">
                                <Trash2 size={14} />
                              </button>
                              <button
                          ...
                          <div className="flex flex-wrap gap-2">
                            {s.items.map((it: ItemType) => (
                              <div
                                key={it.type === "video" ? it.sessionId : it.quizRefId}
                              ...
                              >
                                ...
                                <span className="max-w-[150px] truncate">{it.name}</span>
                                <button
