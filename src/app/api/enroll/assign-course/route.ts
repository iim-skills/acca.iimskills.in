...
function buildStudentMailHtml(data: AnyObject) {
  ...
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111;">
      <h2 style="margin:0 0 12px;">Course Access Confirmed</h2>
      <p>Hi ${data.name},</p>
      <p>Your course access has been activated successfully.</p>
...
function buildAdminMailHtml(data: AnyObject) {
  ...
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111;">
      <h2 style="margin:0 0 12px;">New Course Assignment</h2>
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const paymentStatus = normalizeText(body.payment_status || body.paymentStatus || "").toLowerCase();
    const paymentGateway = normalizeText(body.payment_gateway || body.gateway || "").toLowerCase();
    const paymentId = normalizeText(
      body.payment_id || body.razorpay_payment_id || body.transaction_id || ""
    );
    const orderId = normalizeText(
      body.order_id || body.razorpay_order_id || body.gateway_order_id || ""
    );
    const signature = normalizeText(body.signature || body.razorpay_signature || "");
    ...
    const isFreeCourse = payableFee <= 0;

    if (!isFreeCourse) {
      if (paymentStatus !== "success") {
        return NextResponse.json(
          { error: "Payment not completed. Course cannot be assigned." },
          { status: 400 }
        );
      }
    ...
    const newCourseItem = {
      ...
      payment_status: isFreeCourse ? "free" : "success",
      payment_gateway: paymentGateway || (isFreeCourse ? "free" : ""),
    ...
    try {
      ...
      const sendUserMail = transporter.sendMail({
        from: fromEmail,
        to: email,
        subject: `Course Access Confirmed - ${newCourseItem.course_title}`,
      ...
      const sendAdminMail = adminEmail
        ? transporter.sendMail({
            from: fromEmail,
            to: adminEmail,
            subject: `New Course Assignment - ${name} - ${newCourseItem.course_title}`,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        batchId,
        batchName,
        payment_status: newCourseItem.payment_status,
        payment_gateway: newCourseItem.payment_gateway,
        payment_id: newCourseItem.payment_id,
        order_id: newCourseItem.order_id,
