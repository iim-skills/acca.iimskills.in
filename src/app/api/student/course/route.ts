    ...
    const normalized = courseRows.map((course: AnyObj) => {
      ...
      const mergedModules = modulesArr.map((mod: AnyObj) => {
        ...
        const mergedSubmodules = submodulesArr.map((sub: AnyObj) => {
            ...
            items.forEach((item: AnyObj) => {
              ...
              if (item.type === "video") {
                ...
                const video = videoRows.find(
                  (v: AnyObj) =>
                    String(v.id) === String(item.videoId)
                );
              ...
              if (item.type === "quiz") {
                ...
                const quiz = quizRows.find(
                  (qq: AnyObj) =>
                    String(qq.id) === String(item.quizId)
                );

                mergedItems.push({
                  type: "quiz",
                  quizRefId: item.quizRefId,
                  name: item.name,
                  quizId: item.quizId,
          ...
          if (!items.length && Array.isArray(sub.sessions)) {

            sub.sessions.forEach((ses: AnyObj) => {
          ...
          if (!items.length && Array.isArray(sub.quizzes)) {

            sub.quizzes.forEach((q: AnyObj) => {
