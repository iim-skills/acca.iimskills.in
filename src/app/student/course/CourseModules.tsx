...
export default function CourseModules({
  ...
  const allowedSet = useMemo(() => new Set(((Array.isArray(allowedModules) ? allowedModules : [])).map((x:any) => String(x))), [allowedModules]);
  ...
  useEffect(() => {
    try {
      ...
      if (localStorage.getItem("course_user_key") && !(Array.isArray(allowedModules) && allowedModules.length > 0)) {
        setIsFreeLoggedIn(true); return;
      }
      setIsFreeLoggedIn(false);
    } catch { setIsFreeLoggedIn(false); }
  // SAFER dependency: use JSON.stringify of the allowedModules so we don't call .join on null
